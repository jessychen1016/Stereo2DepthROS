...
void saveTxt(float *data, const int &width, const int &height, const char *name) {
	std::ofstream file(name, std::ios::out);
	for (size_t row = 0; row < height; ++row) {
		for (size_t col = 0; col < width; ++col) {
			int index = col + row * width;
			file << (data[index] > 0 ? data[index] : 0.0) << " ";
		}
		file << "\n";
	}
	file.close();
...
void process (const char* file_1,const char* file_2) {
  ...
  for (int32_t i=0; i<width*height; i++) {
	...
	D1->data[i] = ((255.0*D1_data[i] / disp_max > 0) ? (uint8_t)(255.0*D1_data[i] / disp_max) : 0.0);
	D2->data[i] = ((255.0*D2_data[i] / disp_max > 0) ? (uint8_t)(255.0*D2_data[i] / disp_max) : 0.0);
	//D1->data[i] = ((D1_data[i] > 0) ? (uint8_t)(D1_data[i]) : 0.0);
	//D2->data[i] = ((D2_data[i] > 0) ? (uint8_t)(D2_data[i]) : 0.0);
  }

  // save disparity images
  char output_1[1024], output_11[1024];
  char output_2[1024], output_22[1024];
  ...
  strncpy(output_11, file_1, strlen(file_1) - 4);
  strncpy(output_22, file_2, strlen(file_2) - 4);
  output_11[strlen(file_1) - 4] = '\0';
  output_22[strlen(file_2) - 4] = '\0';
  strcat(output_11, "_disp.txt");
  strcat(output_22, "_disp.txt");
  saveTxt(D1_data, width, height, output_11);
  saveTxt(D2_data, width, height, output_22);
